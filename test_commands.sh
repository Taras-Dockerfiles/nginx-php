...
docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64/v8 --rm -t wujidadi/nginx-php:2.12-php8.3 -t wujidadi/nginx-php:latest . 2>&1 | tee $D/docker-build-np-2.12-php8.3.log

# Create test container and delete it finally
docker run -d -p 50000:80 -it --name Test wujidadi/nginx-php:2.12-php8.3
